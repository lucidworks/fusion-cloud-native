...
| `exporter.configFile`                         | The filename in the docker image that the exporter loads the config from | `solr-exporter-config.xml`
| `exporter.configDir`							| The directory path in the docker image for the exporter to load its config | `/opt/solr/contrib/prometheus-exporter/conf` |
| `exporter.configXml`							| Configuration XML that will be automatically loaded into the exporter | ``
