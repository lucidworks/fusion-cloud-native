...
There are two steps needed to set up Milvus to run on GPU.

. Specify a GPU nodepool
. Use the GPU version of Milvus
...
In your custom values YAML file, at the end of the section for the ml-model-service, add (assuming the name of your GPU nodepool is `gpu-nodepool`):
...
After your cluster is up and running you will need to update the cluster with the GPU version of Milvus as follows:

. Download the current Milvus settings:
`kubernetes get deployment milvus-writable -o yaml > milvus.yaml`
. Edit the settings in the `milvus.yaml` file, which you just created, to specify the GPU version of Milvus.
.. The Milvus version will appear on two lines that will look something like:
... `app.kubernetes.io/version: 0.10.2-*cpu*-d081520-8a2393`
and
... `image: milvusdb/milvus:0.10.2-*cpu*-d081520-8a2393`
.. Edit both lines changing `cpu` to `gpu`:
... `app.kubernetes.io/version: 0.10.2-*gpu*-d081520-8a2393`
and
... `image: milvusdb/milvus:0.10.2-*gpu*-d081520-8a2393`
. Apply the changes:
`kubernetes apply -f milvus.yaml`
