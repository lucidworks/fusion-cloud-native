...
a - You'll need a workstation that has access to DockerHub (hub.docker.com) and can connect to your internal Docker registry, most likely via VPN connection. We'll refer to this computer as `envoy`.

b - Install Docker on `envoy`. You'll need at least 100GB of free disk for Docker.

c - Pull all images from DockerHub to `envoy`’s local registry (will take a long time). Here is a list of the images you need to pull for the Fusion 5.1.1 chart:
...
d - Establish connection from `envoy` to the private Docker registry, most likely this requires you to open a VPN connection. Consult with your internal IT support for help as needed.

We'll refer to the private Docker registry as `<internal-private-registry>` in the steps below.

e - Push images from envoy’s Docker registry into their private registry (will take a long time).
...
f - Install the Docker registry secret into Kubernetes
...
g - Update the custom values yaml for your cluster to point to your private registry and secret for Kubernetes to pull images.
...
```
__Be sure to replace `<internal-private-registry>` with the name of the secret you created in step g above.__
