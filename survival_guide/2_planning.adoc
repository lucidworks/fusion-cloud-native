...
. Pull all images from DockerHub to `envoy`’s local registry (this will take a long time). You can find a list of the images that need to be pulled in the corresponding version file in the images directory.
...
. Generate a values file for services to pull from this private repository
+
For instance:
+
```
./images/generate_values_file.sh values.yaml my-repository docker-registry
```
+
Then pass the outputted file as a new values file to `customize_fusion_values.sh` 
...
```
+
IMPORTANT: Be sure to replace `<internal-private-registry>` with the name of the secret you created in step g above.
