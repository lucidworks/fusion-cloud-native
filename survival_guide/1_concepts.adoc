...
|`fusion-admin` |REST/HTTP |Deployment |`system` |Not required. Minimum of 1 but 2 pods are recommended for HA |Exposes endpoints for admin tasks, such as creating applications and running jobs.
...
|`fusion-indexing` |REST/HTTP |Deployment |`search` or `analytics` depending on write-volume |Yes (CPU or custom metric) |Processes indexing requests.
...
|`query-pipeline` |REST/HTTP |Deployment |`search` |Yes (CPU or custom metric) |Processes query requests.
...
|`kafka` |TCP |StatefulSet |`system` |Yes, required; only 1 pod should be sufficient for most clusters | Topic are used to hold incomming data for solr
