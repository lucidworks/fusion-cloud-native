...
|`connector-plugin-service-<connector_plugin>` | HTTP/TCP |Deployment |`analytics` or `system` |Yes (CPU or custom metric) | Running instance of each connector plugin. *Note:* There is a base deployment `connector-plugin-service` with 0 replicas which is used for deploying each connector plugin. It should not be deleted or scaled.
