...
The latest version of Fusion 5 is 5.8.x. The migration process varies depending on your current version of Fusion.

// tag::body[]

== Migration from Fusion 5.7.x to 5.8.y
...
. This operation changes the solrconfig.xml of all collections in Solr instances. Collections not managed by Fusion in Solr can also be affected.
...
. Use the provided utility Docker image with a script to automate the required collections configuration update:
...
```
[start=2]
. Set `DRY_RUN=true` to see what will be affected without applying any changes to system.
. Check the logs:
...
```
[start=4]
. After the update is done, remove the pod:
...
```
docker run --rm -v $PASSWORD:/apps -e INPUT_FILE=/apps/exported-app.zip lucidworks/fm-upgrade-to-solr:1.0.0
```
This command will produce the exported-app-migrated.zip in the current directory and directly modify the `solrconfig.json` file without creating `configoverlay.json`.
...
This method creates a `configoverlay.json` file in config directory with a modified collection. It will contain all required changes which are applied by Solr on `solrconfig.xml`.
