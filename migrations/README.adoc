...
It is highly advised to perform the operation first on not production environment and compare the `solrconfig.xml` before and after.
...
```
kubectl run --image="lucidworks/fm-upgrade-solrconfig-cache:1.0" \
  -n<namespace> \
...
Instead of providing `HELM_RELEASE` name from which scripts infers the Solr sever name, you can use direct url with `SOLR` variable: `--env="SOLR=http://server:8983"`.
...
```
 docker run --rm -v $(pwd):/apps -e INPUT_FILE=/apps/exported-app.zip fusion-dev-docker.ci-artifactory.lucidworks.com/fm
```

This command will produce the `exported-app-migrated.zip` in current directory.
