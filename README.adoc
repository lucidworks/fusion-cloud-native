...
. Using the AWS console for creating an https://docs.aws.amazon.com/elasticloadbalancing/latest/application/introduction.html[Application Load Balancer^]:

.. Check which port is kubernetes using to allow communication, you can check this listing the fusion services: `kubectl get svc -n <namespace>`.
.. Create an Application Load Balancer that allows communication from port 80/443. You must include your nodes as the target group and the health checker should point to the port kubernetes is using.
.. From here we have 2 options:
... Change the proxy service type from LoadBalancer to Nodeport using the kubernetes port.
... Another option is deploying a nginx ingress and change the proxy service type to ClusterIP.
...
.. Finally, use Route 53 or your DNS provider for creating an ALIAS DNS record to your `<your_dns_name>` pointing to the load balancer DNS name.
