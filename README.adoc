...
=== New to Google Cloud?
You need an account on https://console.cloud.google.com/freetrial/intro[Google Cloud Platform^] before you can begin deploying Fusion on GKE.

If you've already installed the `gcloud` command-line tools, you can skip to the next section.

If you're new to Google Cloud Platform (GCP), then you need to setup a few command-line tools before installing Fusion.
Please refer to the gcloud getting started guide to install the required tools: https://cloud.google.com/sdk/docs/quickstarts[Google Cloud SDK^]
...
You should also install `Helm`. On MacOS, you can do:
```
...
```
./setup_f5_gke.sh -c <existing_cluster> -p <gcp_project_id> -r <release> -n <namespace> --values example-values/affinity.yaml --values example-values/resources.yaml
```
The `--values` option can be passed multiple times, if the same configuration property
is contained within multiple `values` files then the values from the latest file passed as a `--values` option
are used.
