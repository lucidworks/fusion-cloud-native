...
Fusion 5.0.1 runs Solr 8.2.0 and Zookeeper 3.4.14. Prior to upgrading to Fusion 5.0.2, you need to upgrade Solr to 8.3.1 in your existing cluster and perform some minor changes to the custom values yaml.

Helm V3 no longer uses the server-side Tiller component with all of its security concerns and problems. Prior to upgrading, list our your releases for Helm v2:
...
So plan to have a minute or so of downtime in this cluster. Also, to avoid as much downtime as possible, be ready to do step 7 immediately after deleting the existing statefulset.
...
Deleting the StatefulSet does not remove the persistent volumes backing Zookeeper, which is how we can perform an upgrade w/o losing data.
...
./setup_f5_gke.sh -c <CLUSTER> -p <PROJECT> -z <ZONE> \
  -n <NAMESPACE> -r <RELEASE> \
    --values <MY_VALUES> --upgrade --force
```
