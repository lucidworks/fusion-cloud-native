...
If you're working with Helm directly and not using one of our setup scripts, then run the `customize_fusion_values.sh` script to create a custom values yaml file from our `customize_fusion_values.yaml.example` template as a starting point (pass `--help` for usage details):
```
...
After making changes to the custom values yaml file, run an upgrade.
...
Once you're ready to build a Fusion cluster for production, please see the https://github.com/lucidworks/fusion-cloud-native/blob/master/Fusion_5_Survival_Guide_v0.1.pdf[Fusion 5 Survival Guide] PDF in this repo.
