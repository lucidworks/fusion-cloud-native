...
// tag::upgrade-gke[]
*Before you begin: for some circumstances, you will need to perform additional or different steps*:

* Currently running Fusion 5.0.1? Use the instructions at <<upgrade-to-502,Upgrade from 5.0.1>> instead of the below
instructions.

* Upgrading from a version before release 5.0.3-3 to release 5.0.3-3 or higher? Delete the logstash statefulset before following the below instructions;
`kubectl delete sts <RELEASE>-logstash`. The logstash statefulset will get re-created during the upgrade; this operation
does not delete the PVC, so the data will remain intact.
