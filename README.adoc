...
Set each connector's resource values in the `connector-plugin` section. The following code sample shows an example.
...
<2> The resources settings. You must specify the limits, the requests, and the CPU for each.

*IMPORTANT* After editing the `connector-plugin` section, the affected connector must be re-installed.
